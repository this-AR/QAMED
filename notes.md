...
## Phase 4.1: Ingestion Bug Fixes (Hotfixes)

### Relative Import Path Fix
- **Decision:** Added `sys.path.append` to the top of `data/ingest.py` to include the project root.
- **Reasoning:** Running `python data/ingest.py` directly caused a `ModuleNotFoundError` because Python couldn't find `config.py` in the parent directory. Dynamically appending the parent directory allows the script to be run easily without the `-m` flag.

### Qdrant Payload Index for Deletion
- **Decision:** Modified `ensure_collection()` in `data/ingest.py` to explicitly create a `keyword` payload index on the `book_name` field.
- **Reasoning:** Qdrant enforces that filtering operations (like deleting chunks by `book_name` during a `--force` re-ingestion) require a payload index. Automatically creating this index ensures that old chunks are successfully deleted before new ones are inserted.
...
## Phase 5: Automated Evaluation, Benchmarking & Architectural Overhaul (v2.5 - v2.7)

### 1. Initial Benchmark (v2.5): Hybrid vs. Pure Dense
- **Context:** We built an automated RAGAS benchmark (`evaluation/benchmark.py`) to run 4 curated medical questions against 6 different configurations (Hybrid vs. Dense × Prompt v1, v2, v3). The goal was to establish a baseline for latency, faithfulness, and hallucination guardrail triggers.
- **What Happened:** The initial results showed that Hybrid search (BM25 + Dense + RRF) took a massive **~23.3 seconds** on average to retrieve context. Pure Dense search was faster at **~16.8 seconds**. Both methods returned 75% of the exact same chunks (Overlap Ratio: 0.75).
- **Initial Decision:** Because Dense was significantly faster and retrieved mostly the same documents as Hybrid for structured textbook data, I initially recommended dropping Hybrid search to save ~7 seconds of latency.
- **The Tradeoff:** While Dense was faster, Hybrid search paired with the `v3` ("Expert Medical Synthesis") prompt yielded the highest absolute faithfulness score (0.72) in the RAGAS evaluation, proving that BM25 was still capturing critical exact-match terminology that dense vectors occasionally missed.

### 2. Optimized Benchmark & Reranker Swap (v2.6)
- **Context:** Even with Pure Dense search, ~17 seconds of retrieval latency was completely unacceptable for a production web application. The bottleneck was identified as the Reranker step.
- **What Was Removed & Why:** 
  - We removed the heavy `BAAI/bge-reranker-base` model (278 million parameters) in `services/retrieval.py`. It was simply too slow for CPU/standard environments when scoring 75 documents per query.
  - We reduced the `DENSE_TOP_K` config variable from `75` down to `30`. Retrieving 75 documents from Qdrant just to rerank them down to 6 was excessive and mathematically wasteful.
- **What Was Added:** 
  - We swapped the reranker to `cross-encoder/ms-marco-MiniLM-L-6-v2` (only 22 million parameters). It is ~10x lighter and specifically trained on MS-MARCO for rapid question-answering retrieval.
- **What Happened:** The second benchmark run with these optimizations yielded massive success. Retrieval latency across the board dropped from ~17 seconds down to **~3.7 seconds** (a ~4.5x speedup).
...
- **Final Decision on Hybrid:** Because retrieval was now blazing fast (under 4 seconds), the latency penalty of Hybrid search was effectively eliminated (Hybrid ran in 3.5s, Dense in 3.8s in the new benchmark). Furthermore, chunk overlap jumped to 92%, and the `Hybrid + v3` config hit an all-time high faithfulness score of **0.8636**. 
  - *Action:* `USE_HYBRID_SEARCH` was permanently re-enabled as the default in `config.py` because we can now afford the accuracy boost without sacrificing speed.

### 3. Guardrail Overhaul (v2.7)
- **Context:** The automated benchmark revealed a critical flaw in the output hallucination guardrail (`services/guardrails.py`). The `v3` prompt consistently generated the highest quality, most comprehensive answers, but the LLM-based guardrail kept flagging them as "HALLUCINATED".
- **What Was Removed & Why:** The pure LLM-based guardrail (which made a secondary Groq API call to check if the answer was supported) was too binary and rigid. Because `v3` encourages fluid, expert synthesis rather than verbatim quoting, the LLM guardrail threw false positives, triggering unnecessary retries and degrading UX.
- **What Was Added:** We implemented a custom **Two-Tier Mathematical Guardrail**.
  - **Tier 1 (Fast Cosine):** We split the generated answer into individual sentences using `pysbd`. We embed each sentence (reusing the already-loaded `HuggingFaceEmbeddings` model to cost zero extra latency/memory) and compute the exact Cosine Similarity against the retrieved context chunks. If the average max similarity is ≥ `0.30`, it mathematically proves the answer is grounded. It instantly passes.
  - **Tier 2 (LLM Fallback):** If the cosine similarity falls into an ambiguous "grey zone" (between 0.18 and 0.30), only *then* do we fallback to the original Groq LLM check for a semantic second opinion.
- **Decision & Impact:** By shifting the primary hallucination check to an objective mathematical threshold, we completely eliminated the false-positive flags against the `v3` prompt. This also saves a full Groq API round-trip on >90% of queries, saving token costs and reducing total generation latency by several seconds.

### 4. LLMLingua Context Compression Evaluation (v2.8)
- **Context:** Even with highly targeted retrieval (Top K = 6 + Parent Expansion), we are passing massive chunks of text (up to 3,000 tokens) into the Groq API on every question. This increases TTFT (Time-To-First-Token) latency and distracts the LLM.
- **What We Tested:** We wrote a standalone script (`evaluation/test_llmlingua.py`) to run `microsoft/llmlingua-2-bert-base-multilingual-cased-meetingbank` against our raw medical textbook chunks.
- **Results:**
  - **Token Savings:** The compressor achieved an average compression ratio of **28.9% of original size**. This means a massive **~71% reduction in tokens sent to the LLM**.
  - **Latency Tradeoff:** The compression step took ~1.2s to ~2.3s locally on CPU. In a cloud environment with minimal GPU, this will drop to milliseconds.

  | Test Case | Original Tokens | Compressed Tokens | Compression Ratio | Token Savings | Compression Time (CPU) |
  | :--- | :--- | :--- | :--- | :--- | :--- |
  | Inguinal Canal | ~317 | ~85 | 26.8% | 73.2% | 2.37s |
  | Renal Arteries | ~187 | ~58 | 31.0% | 69.0% | 1.26s |
  | **Average** | **-** | **-** | **28.9%** | **71.1%** | **1.81s** |

  - **Quality & Text Preview:** Qualitative review showed that it flawlessly preserved the core medical entities while stripping syntactic fluff.
    > *Original text snippet:* "The inguinal canal is a narrow oblique passage in the lower part of the anterior abdominal wall. It extends from the deep inguinal ring..."
    > *Compressed snippet:* "inguinal canal narrow oblique passage anterior abdominal wall extends from deep inguinal ring... transmits spermatic cord round ligament..."
- **Decision:** **SKIP (For Now).** While the token savings are impressive, running the compressor locally on a CPU takes ~1.8s. Because Groq's LPU processes tokens so fast (~500-800 tps), it only takes Groq ~1.0s to read those extra tokens anyway. Therefore, the CPU compression step actually *adds* ~0.8s of net latency to the pipeline. We will keep the script for future GPU-based deployments, but leave it out of the active retrieval pipeline to maximize speed.
